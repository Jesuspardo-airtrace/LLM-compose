...
import os
from litellm import completion  # Se importa para realizar solicitudes a LiteLLM

os.environ['DEEPSEEK_API_KEY'] = 'XXXXX'
...
def generate_llm_response(model_name, messages):
    """Helper function to call LiteLLM API with Deepseek model."""
    response = completion(
        model=model_name,  # Nombre del modelo Deepseek en LiteLLM
        messages=messages
    )
...
qa_generator = Agent(
    ...
    llm={
        "model": "deepseek/deepseek-chat",  # Usando el modelo de Deepseek en LiteLLM
        "api_base": "http://localhost:11434",  # Asegúrate de que el servidor esté corriendo en este puerto
    },
...
total_questions_evaluator = Agent(
    ...
    llm={
        "model": "deepseek/deepseek-chat",  # Modelo Deepseek en LiteLLM
...
cot_generator = Agent(
    ...
    tools=[cot_save],
    llm={
        "model": "deepseek/deepseek-reasoner",  # Usando el modelo Deepseek con razonamiento
        "api_base": "http://localhost:11434",
    },
    verbose=False
)
...
upload_to_huggingface = Agent(
    ...
    llm={
        "model": "deepseek/deepseek-reasoner",  # DeepSeek con razonamiento
        "api_base": "http://localhost:11434",
    },
    verbose=False
)
...
upload_to_huggingface_task = Task(
    ...
    2. Upload to Jesus-airtrace/cot-dataset""",
